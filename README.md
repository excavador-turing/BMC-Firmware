...
| **A firmware update no longer touches running nodes.** bmcd reads the live rail state on start and adopts it instead of re-applying what `bmcd.bin` persisted; a cold boot still restores the persisted state | Ten flashes and daemon restarts with four nodes powered: every rail stayed on, every `/proc/uptime` monotonic, no node dropped a ping. Fixes upstream [bmcd#90](https://github.com/turing-machines/bmcd/issues/90) |
...
| **The image is 78 % of its UBI slot** — 37,003,264 of 46,981,120 bytes — and the build **fails at 90 %** | Down from 37,326,848 while *gaining* a serial console, four new panels and a newer toolchain |
...
| **The scrape credential is no longer the root password.** `/metrics` shared an authenticator with `/api/bmc`, and that authenticator checks `/etc/shadow` with no per-route authorization behind it -- so the one credential that could scrape this board could also power four modules off and flash it | Measured after the flash: the token returns **200** on `/metrics` and **401** on `/api/bmc`; root returns **401** on `/metrics`. The second of those is the one that matters, and note the board's own health gate cannot see any of it -- it checks that bmcd answers and the switch ports exist, and a completely broken `/metrics` passes both |
| **The board reports whether a newer release exists** | `type=update_check` runs `tpi-selfupdate --check --json`, so the interface and the updater cannot disagree about what an upgrade would install. Cached an hour; a board with no route out reports the reason rather than claiming it is current |
| **The Firmware Upgrade page names the version that is staged**, not merely that something is. The staging volume is static and never mounted, so its version cannot be read on a running system; the stager records the tag, checksum and time to `/mnt/overlay/staged-firmware` instead, and the promotion script removes it once it has decided | Proven twice. By mutation -- no note reports `staged: null`, a note reports every field, removing it returns to `null` -- and then for real: staging v2.5.0 wrote `VERSION=v2.5.0` with the checksum the updater had just verified, and the promotion script cleared it on its own. The page renders `Staged version v2.5.0 · tpi-selfupdate`. The writer and the clearer are both present in the running image |
